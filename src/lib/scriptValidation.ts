...
const RESIDUE_PATTERNS: Array<[RegExp, string]> = [
  ...
  [/\b(?:generating part|writing part|continue from|draft continues|unfinished|placeholder|debug)\b/i, 'generation residue'],
  [/\b(?:stage output|scene card|linter report|qa notes|prompt notes|output start|output end)\b/i, 'technical residue'],
  [/^\s*(?:#{1,6}\s+|={3,}|-{3,}|\*{3,})/i, 'heading or decorative separator'],
  [/^\s*(?:stage|scene|part)\s+(?:one|two|three|four|five|six|seven|eight|nine|ten|\d+)\b\s*[:.-]/i, 'stage/scene/part label'],
  [/^\s*(?:current stage|status|what i checked|problems found|what i fixed|next action)\s*:/i, 'operator report residue'],
];

const AVATAR_PATTERN = /^\s*(?:\[(?:AVATAR|COMMENTARY|HOST|NARRATOR)\]|(?:AVATAR|COMMENTARY|HOST)\s*:)/i;
const CYRILLIC_PATTERN = /[\u0400-\u04FF]/;
const FIRST_PERSON_PATTERN = /\b(?:I|me|my|mine|myself|we|us|our|ours|ourselves)\b/i;
const DIGIT_PATTERN = /\d/;
const ROBOTIC_OPENING_PATTERN = /^(?:then i|after that|next,?\s+i|i started to|i began to|i decided to|i realized|i knew|i saw|i looked)\b/i;
...
const AUTHORIAL_CLICHE_PATTERNS: Array<[RegExp, string]> = [
  ...
  [/\bwhat happened next (?:would|could) change\b/i, 'what happened next would change'],
  [/\bwith a heavy heart\b/i, 'with a heavy heart'],
  [/\ba shiver (?:ran|went) down\b/i, 'shiver ran down'],
  [/\ba wave of (?:fear|relief|dread|panic|satisfaction|horror)\b/i, 'wave of named emotion'],
...
function addIssue(issues: ScriptValidationIssue[], issue: ScriptValidationIssue) {
  issues.push(issue);
}

export function validateScriptText(text: string, scope: ScriptValidationScope): ScriptValidationResult {
    ...
    if (!isAvatar) {
      ...
      if (ROBOTIC_OPENING_PATTERN.test(paragraph)) {
        ...
        const flatOpening = paragraph
          ...
          .replace(/[^\p{L}\s']/gu, '');
        if (flatOpening) {
          flatOpeningMap.set(flatOpening, (flatOpeningMap.get(flatOpening) || 0) + 1);
        }
      }
  ...
  const repeatedFlatOpening = [...flatOpeningMap.entries()].find(([, count]) => count >= 3);
  ...
  if (normalParagraphCount > 0) {
    ...
    if (firstPersonParagraphCount === 0) {
      addIssue(issues, {
        ...
        message: 'No normal paragraph uses first-person voice. Script must read like a living first-person recap.',
      });
  ...
  if (scope === 'clean_export' && paragraphs.some((paragraph) => /^#{1,6}\s+|^Part\s+\d+\s*:/i.test(paragraph))) {
    addIssue(issues, {
      severity: 'fail',
      code: 'export_headings',
      message: 'Clean export contains headings or part labels. Final voiceover export must be clean narration.',
    });
  }
  ...
  return {
    ...
    hasDuplicateBlocks,
    firstPersonCoverage: normalParagraphCount > 0 ? firstPersonParagraphCount / normalParagraphCount : 0,
  };
}

function worstStatus(left: SupervisorStatus, right: SupervisorStatus): SupervisorStatus {
...
export function mergeSupervisorReportWithValidation(
  ...
  const validationFixes = validation.failures.map((issue) => {
    ...
    if (issue.code === 'technical_residue' || issue.code === 'export_headings') {
      return 'Remove technical labels, headings, debug notes, and export residue.';
    }
    if (issue.code === 'duplicate_paragraph') {
      return 'Remove duplicate paragraphs and keep only the stronger version.';
    }
    if (issue.code === 'generic_ai_cliche') {
      return 'Replace generic AI/cliche phrasing with concrete physical action, tactical observation, or a specific first-person decision.';
    }
    if (issue.code === 'robotic_sequence_rhythm' || issue.code === 'repetitive_flat_openings') {
      return 'Run an Author Voice Pass: vary paragraph openings, remove flat sequence transitions, and make each beat enter through pressure, action, calculation, or consequence.';
    }
...
export function validationIssueSummary(validation: ScriptValidationResult, maxItems = 4): string {
