...
// Replace finding specific chunk
const regex = /stageFiveScriptWriterPrompt:\s*`([\s\S]*?)(?=\n\s*`,\n\s*stageSixCleanExportPrompt)/;

if (regex.test(content)) {
  const updated = content.replace(regex, `stageFiveScriptWriterPrompt: \`${newPrompt}`);
